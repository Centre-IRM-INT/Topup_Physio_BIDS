		...
		mkdir -p ${OUTDIR}
     	...
     	if [ $num == 1 ];then
			...
			if [ `expr $dimz % 2` -eq 1 ]; then
    			...
    			topup --imain="${OUTDIR}/${sub}_FieldmapAP_PA" --datain="${OUTDIR}/${sub}_acqparamsAP_PA.txt" --config=/Users/jsein/hubiC/JUL/Topup_Physio_BIDS/b02b0_oddslices.cnf --out="${OUTDIR}/${sub}_my_topup_results1" --fout="${SUBDIR}fmap/${sub}_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images"
...
#
			...
			if [ `expr $dimz % 2` -eq 1 ]; then
    			...
    			# to use topup config file for odd slices uncomment line below and comment lines above
    			echo "odd number of slices: run topup with specific config file"
    			topup --imain="${OUTDIR}/${sub}_Fieldmap1AP_PA" --datain="${OUTDIR}/${sub}_acqparams1AP_PA.txt" --config="../Topup_Physio_BIDS/b02b0_oddslices.cnf" --out="${OUTDIR}/${sub}_my_topup_results1" --fout="${SUBDIR}fmap/${sub}_run-1_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images1"
 			else       
 				topup --imain="${OUTDIR}/${sub}_Fieldmap1AP_PA" --datain="${OUTDIR}/${sub}_acqparams1AP_PA.txt" --config=b02b0.cnf --out="${OUTDIR}/${sub}_my_topup_results1" --fout="${SUBDIR}fmap/${sub}_run-1_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images1"
			fi
			...
			if [ `expr $dimz % 2` -eq 1 ]; then
    			...
    			# to use topup config file for odd slices uncomment line below and comment lines above
    			echo "odd number of slices: run topup with specific config file"
    			topup --imain="${OUTDIR}/${sub}_Fieldmap2AP_PA" --datain="${OUTDIR}/${sub}_acqparams2AP_PA.txt" --config="../Topup_Physio_BIDS/b02b0_oddslices.cnf" --out="${OUTDIR}/${sub}_my_topup_results2" --fout="${SUBDIR}fmap/${sub}_run-2_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images2"
 			else       
    			topup --imain="${OUTDIR}/${sub}_Fieldmap2AP_PA" --datain="${OUTDIR}/${sub}_acqparams2AP_PA.txt" --config=b02b0.cnf --out="${OUTDIR}/${sub}_my_topup_results2" --fout="${SUBDIR}fmap/${sub}_run-2_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images2"
			fi
		...
		pushd  ${SUBDIR}$ses/fmap
     	...
     	if [ $num == 1 ];then
			...
			fslmerge -t "${OUTDIR}/${sub}_${ses}_FieldmapAP_PA"  "${SUBDIR}$ses/fmap/${sub}_${ses}_dir-AP_epi.nii.gz" "${SUBDIR}$ses/fmap/${sub}_${ses}_dir-PA_epi.nii.gz"
			...
			echo -e   "$phase1 $RT\n$phase1 $RT\n$phase1 $RT\n$phase2 $RT\n$phase2 $RT\n$phase2 $RT" > ${OUTDIR}/${sub}_${ses}_acqparamsAP_PA.txt

			# Correct for EVEN number of slices to use b02b0.cnf
			dimz=`fslval ${OUTDIR}/${sub}_${ses}_FieldmapAP_PA.nii.gz dim3`
			if [ `expr $dimz % 2` -eq 1 ]; then
    			echo "remove one slice to get odd number of slices"
    			fslroi ${OUTDIR}/${sub}_${ses}_FieldmapAP_PA.nii.gz ${OUTDIR}/${sub}_${ses}_FieldmapAP_PA.nii.gz 0 -1 0 -1 1 -1 0 -1
    			topup --imain="${OUTDIR}/${sub}_${ses}_FieldmapAP_PA" --datain="${OUTDIR}/${sub}_${ses}_acqparamsAP_PA.txt" --config=b02b0.cnf --out="${OUTDIR}/${sub}_${ses}_my_topup_results" --fout="${SUBDIR}$ses/fmap/${sub}_${ses}_fieldmap" --iout="${OUTDIR}/${sub}_${ses}_my_unwarped_images"
    			# to use topup config file for odd slices uncomment line below and comment lines above
    			#echo "odd number of slices: run topup with specific config file"
    			#topup --imain="${OUTDIR}/${sub}_FieldmapAP_PA" --datain="${OUTDIR}/${sub}_acqparamsAP_PA.txt" --config=/Users/jsein/hubiC/JUL/Topup_Physio_BIDS/b02b0_oddslices.cnf --out="${OUTDIR}/${sub}_my_topup_results1" --fout="${SUBDIR}$ses/fmap/${sub}_fieldmap" --iout="${OUTDIR}/${sub}_my_unwarped_images"
			...
			fslmaths "${OUTDIR}/${sub}_${ses}_my_unwarped_images" -Tmean "${SUBDIR}$ses/fmap/${sub}_${ses}_magnitude"
			...
			jq 'del(.IntendedFor)' "${SUBDIR}$ses/"fmap/"${sub}_fieldmap.json"  > "${OUTDIR}/tmp.$$.json" && mv  "${OUTDIR}/tmp.$$.json" "${SUBDIR}/"fmap/"${sub}_fieldmap.json"
			...
			do
        		...
        		then
					...
					jq '. + { "B0FieldIdentifier": "B0map"}' "${SUBDIR}$ses/"fmap/"${sub}_${ses}_dir-AP_epi.json" > "${OUTDIR}/tmp.$$.json" && mv  "${OUTDIR}/tmp.$$.json" "${SUBDIR}$ses/"fmap/"${sub}_${ses}_dir-AP_epi.json"
					jq '. + { "B0FieldIdentifier": "B0map"}' "${SUBDIR}$ses/"fmap/"${sub}_${ses}_dir-PA_epi.json" > "${OUTDIR}/tmp.$$.json" && mv  "${OUTDIR}/tmp.$$.json" "${SUBDIR}$ses/"fmap/"${sub}_${ses}_dir-PA_epi.json"
					jq '. + { "B0FieldSource": "B0map"}' "${func//nii.gz/json}" > "${OUTDIR}/tmp.$$.json" && mv  "${OUTDIR}/tmp.$$.json" "${func//nii.gz/json}"
					jq 'del(.B0FieldSource)' "${SUBDIR}$ses/"fmap/"${sub}_${ses}_fieldmap.json"  > "${OUTDIR}/tmp.$$.json" && mv  "${OUTDIR}/tmp.$$.json" "${SUBDIR}$ses/"fmap/"${sub}_${ses}_fieldmap.json"
				fi
			done
     	elif [ $num == 2 ];then
